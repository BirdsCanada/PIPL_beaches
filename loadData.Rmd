...
landuse<-read.csv(file = '../Data/landuse_intersection.csv') %>% 
  select(1:21,36,52:54,64:67,72) %>% 
  select_if(all_na)%>% 
  rename(Description = descriptio, BeachID = Name, Int_Area = Int_Area2)
...
x$tot_area<-ifelse(is.na(x$landclass), x$tot_area_beach, x$tot_area)
