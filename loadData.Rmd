...
beachnames<-append(beachnames, c("James", "Bowen Island", "Goose and Burks Point", "Goose Haven", "Taylor Head Bay", "Chance Harbour")) #
...
beachArea$BeachID %<>% 
  ...
  gsub("Gull Island Beach", paste(beachnames[60]), .) #index 60 is white point, renaming gull island since they're the same beach but most of the data uses white point beach.
...
beachPhys$Management<-ifelse(beachPhys$BeachID %in% c("Taylor Head Bay"), "Provincial", beachPhys$Management)
#beachPhys$Management<-ifelse(beachPhys$BeachID %in% c(??), "Federal", beachPhys$Management)
#beachPhys$Management<-ifelse(beachPhys$BeachID %in% c(??), "Municipal", beachPhys$Management)

```

minor changes to landuse data
```{r landuse data}
setdiff(landuse$Name, beachnames) #some problems
...
#manually assigning names that aren't working with gsub
#port joli (goose haven) 271-275
x<-which(landuse$Name == "Port Joli (Goose Haven)") #find all rows with the wrong name
landuse[paste(x), 'Name']<-"Goose Haven" #replace with correct name beachnames[64]
...
x<-landuse %>% 
  ...
  summarize(tot_area = sum(Int_Area), tot_area_km = tot_area/1000000, percentage = tot_area_km/(pi*(30^2))) #currently 30km buffer region data

beachPhys<-inner_join(beachPhys, x, by = 'BeachID')
```
