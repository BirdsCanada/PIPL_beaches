...
beachnames<-append(beachnames, c("James", "Bowen Island", "Goose and Burks Point", "Goose Haven", "Taylor Head Bay", "Chance Harbour", "Margaree Harbour", "MacKay's Point", "Port Mouton Island", "Jimtown", "Linwood", "South Lakevale (Cribbons)", "Tracadie Big Island / Delorey", "Tracadie West Arm")) #
...
brc1$BeachID %<>% 
  ...
  gsub("Ratcliffe Hills (The Cape)", paste(beachnames[58]), .) %>% #not working??
...
brc2$BeachID %<>% 
  ...
  gsub("Dominion (Lingan) Beach", paste(beachnames[18]), .) %>%  #notworking
  ...
  gsub("Pictou Split Bar (Lighthouse)", paste(beachnames[42]), .) %>%  #notworking
  gsub("Rainbow Haven", paste(beachnames[45]), .) %>%
  gsub("Round Bay and Roseway", paste(beachnames[48]), .) %>%
  gsub("Sandhills PP", paste(beachnames[49]), .) %>% #notworking
  gsub("Shipping Point (Port Hood)", paste(beachnames[51]), .) %>%  #notworking
  gsub("South Harbour ", paste(beachnames[52]), .)
  
summary(as.factor(brc2$BeachID)) #looks good
summary(as.factor(brc1$BeachID))
...
left_join(prodData, beachLength, by = 'BeachID')
```
