...
plot_models(m2, mp2, m.labels = c("Productivity", "Breeding Pairs"), show.p = TRUE, p.shape = FALSE, show.values = TRUE, spacing = .7, dot.size = 1.2, value.size = 3, axis.labels = c("Region","Developed Land","Foraging Area","Border Road","Part Border Road","No Border Road","Population Density","Parking Area","Beach Length","Beach Width","Beach Area"))
...
#(saved 7x8 landscape)
plot_models(mf2, mb2, m.labels = c("Productivity", "Breeding Pairs"), show.p = TRUE, p.shape = FALSE, show.values = TRUE, spacing = .7, dot.size = 1.2, value.size = 4.5, axis.labels = c("Fenced Nests","Mean Unleashed Dogs","Vehicles","Visitor Intensity","Mean Total Dogs","Large Signs","Small Signs"))
```
...
fig.data<-mod_human %>% 
  ...
  na.omit()
fig.data<-left_join(fig.data, mod_phys[,c(1:4, 7, 9, 25,27)], by = "BeachID")
...
x<-fig.data %>% 
  group_by(BeachID) %>% 
  summarize(meanIntensity = mean(peopleIntensity), developedLand = mean(developedLand), region = Region)

ggplot(fig.data, aes(developedLand, peopleIntensity, color = County)) + geom_point()
ggplot(x, aes(developedLand, meanIntensity, color = region)) + geom_point() + geom_smooth(method = "lm")
```
