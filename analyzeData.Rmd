...
lm(tot_area_km_Beach ~ bufferPop_km2, data = mod_phys) #n.s.
...
```{r plotting effect}
sjPlot::plot_model(mb1, show.values = TRUE, axis.lim = c(.001,1000), sort.est = TRUE, value.offset = .3)
```
...
```{r effect size plot}
sjPlot::plot_model(mf1, show.values = TRUE, axis.lim = c(.001,1000), sort.est = TRUE, value.offset = .3)
```
